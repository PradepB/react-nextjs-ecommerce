...
const fetchAllCart = async () => {
  const allCart = await fetch(`https://fakestoreapi.com/carts/user/3`);
  const allCartData = await allCart.json();
  return allCartData
}
...
console.log(params)
  const singleCart = await fetchCart(params.productId);
  const allCart = await fetchAllCart();
